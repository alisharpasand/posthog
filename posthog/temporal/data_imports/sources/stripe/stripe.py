...
class _RateLimitRetryingRequestsClient(RequestsClient):
    ...
    run. We also retry a 2xx whose list body was truncated mid-stream — Stripe surfaces that as a
    JSON decode failure (``Invalid response body from API``) only after the SDK's retry loop, too
    late for it to recover on its own. Our Stripe reads are list/GET calls, so retrying them is
    idempotent."""
    ...
    def _should_retry(
        ...
        # table are a 429 (the SDK omits it) and a 2xx with a truncated list body (the SDK only fails
        # on it later, while parsing) — both safe to retry for our idempotent reads.
