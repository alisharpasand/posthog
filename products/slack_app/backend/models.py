...
class SlackSettings(UUIDModel):
    ...
    # AI settings feeding into task-run sandboxes. (runtime_adapter, model)
    ...
    # in the AI settings service to keep heavy task-layer imports off the
    # Django app-load path.
    runtime_adapter = models.CharField(max_length=32, null=True, blank=True)
    model = models.CharField(max_length=128, null=True, blank=True)
    reasoning_effort = models.CharField(max_length=16, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
