            ...
            .filter_map(|row| {
                match serde_json::from_value(row.filters) {
                    Ok(filters) => Some(FeatureFlag {
                        ...
                        has_experiment: Some(row.has_experiment),
                    }),
                    Err(e) => {
...
mod tests {
    ...
    async fn test_from_pg_computes_has_experiment() {
        ...
        assert_eq!(by_key("flag_with_live_experiment"), Some(true));
        assert_eq!(by_key("flag_with_deleted_experiment"), Some(false));
        assert_eq!(by_key("flag_without_experiment"), Some(false));
    }
